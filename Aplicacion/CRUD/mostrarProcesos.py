...
def obtener_corrales_animales_clientes(request, id):
    try:
...
def AgregarMovimientoAnimales(request, ID):
    ...
    FiltradoCorral = tblCorrales.objects.filter(IDCliente = Cliente)
    FiltradoMovimiento = tblTipoMov.objects.get(ID=Movimiento)
    FTipoAnimal = tblAnimalesTipo.objects.all()
    Detalle = tblDetalleMovAnimales.objects.filter(IDFolio=folio).values('IDFolio','IDCorral_id__Descripcion', 'IDAnimales_id__Descripcion', 'Cantidad', 'PesoPromedio', 'PesoTotal' )
    ServiciosWeb = servicioActivo()
...
def detalleAnimales(request, ID):
    ...
    AgMovimientos = tblMovimientoAnimales.objects.get(Folio=ID)
    folio = AgMovimientos.Folio
    Detalle = tblDetalleMovAnimales.objects.filter(IDFolio=folio).values('ID', 'IDFolio', 'IDCorral_id__Descripcion', 'IDAnimales_id__Descripcion', 'Cantidad', 'PesoTotal', 'PesoPromedio')
    ServiciosWeb = servicioActivo()
